...
builder.Services.Configure<CodeChangeOptions>(
    builder.Configuration.GetSection(CodeChangeOptions.SectionName));
...
builder.Services.AddSingleton<CodeChangeService>();
...
// ---- POST /api/code/changes/preview ------------------------------------------
// "Make changes to the code": reads a local file, asks the LLM to fold the
// review's suggestions in, and returns the FULL proposed file. Writes nothing —
// the frontend shows the result and the user confirms before /apply.
app.MapPost("/api/code/changes/preview", async (
    CodeChangePreviewRequest req,
    CodeChangeService codeChange,
    ProviderValidator validator,
    DotEnvFile dotEnv,
    CancellationToken ct) =>
{
    ...
    if (string.IsNullOrEmpty(provider)
        || (provider != "openai" && provider != "openrouter"))
    {
        return Results.Ok(new CodeChangePreviewResult(
            false, "no-key", null, null, null, null, null, false, null,
            "Provider must be 'openai' or 'openrouter'."));
    }
    ...
    var result = await codeChange.PreviewAsync(req, provider, req.Model ?? cfg.Model, apiKey, ct);
    return Results.Ok(result);
});

// ---- POST /api/code/changes/apply ------------------------------------------------
// Writes the user-confirmed file to disk after a staleness check, keeping the
// original as <file>.bak. No provider/LLM involved.
app.MapPost("/api/code/changes/apply", async (
    CodeChangeApplyRequest req,
    CodeChangeService codeChange,
    CancellationToken ct) =>
{
    var result = await codeChange.ApplyAsync(req, ct);
    return Results.Ok(result);
});
