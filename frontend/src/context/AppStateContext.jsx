...
import {
  postCodeChangesPreview,
  postCodeChangesApply,
} from '../lib/codeChangeApi.js';
...
const initialState = {
  ...
  // "Make changes to the code" — a proposed full-file rewrite awaiting the
  // user's "Write to disk" confirmation. Cleared once written or discarded.
  // { path, original, proposed, originalHash, summary } | null
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'CODE_CHANGE_START':
      return {
        ...state,
        status: 'pending',
        pendingStatusIndex: 0,
        statusLabel: action.label ?? 'Preparing changes…',
      };

    case 'CODE_CHANGE_PREVIEW': {
      const { path, original, proposed, originalHash, summary, language, changed } =
        action.preview;
      const filename = basename(path);
      const note = changed
        ? `Proposed changes to ${filename}. ${summary} Review the full new version on the right, then choose “Write to disk”.`
        : `No changes were needed for ${filename}. ${summary}`;
      return {
        ...state,
        status: 'idle',
        statusLabel: null,
        messages: [
          ...state.messages,
          { id: nextId(), role: 'assistant', text: note, findings: [], suggestions: [] },
        ],
        pendingCodeChange: changed
          ? { path, original, proposed, originalHash, summary }
          : null,
        panel: {
          expanded: true,
          kind: 'code',
          code: { filename, language: language ?? 'text', content: proposed },
          pr: null,
          suggestions: null,
        },
      };
    ...
    case 'CODE_CHANGE_APPLIED': {
      const { path, backupPath, bytesWritten } = action.result;
      return {
        ...state,
        status: 'idle',
        statusLabel: null,
        ...
        messages: [
          ...state.messages,
          {
            ...
            role: 'assistant',
            text:
              `Wrote ${bytesWritten} byte${bytesWritten === 1 ? '' : 's'} to ${path}. ` +
              `The previous version was kept at ${backupPath}.`,
            findings: [],
            suggestions: [],
          },
        ],
...
export function AppStateProvider({ children }) {
  ...
  // "Make changes to the code" — the newest chat turn that carried code
  // suggestions is what gets applied; its findings ride along as context.
  const latestReview = (messages, panel) => {
    for (let i = messages.length - 1; i >= 0; i -= 1) {
      const m = messages[i];
      if (Array.isArray(m.suggestions) && m.suggestions.length > 0) {
        return { suggestions: m.suggestions, findings: m.findings ?? [] };
      }
    }
    return { suggestions: panel.suggestions ?? [], findings: [] };
  };

  // Step 1 — ask the backend for a full-file rewrite with the review's
  // suggestions folded in. Nothing is written yet. Returns { ok } so the path
  // dialog can stay open and show the error on failure.
  const previewCodeChanges = useCallback(async (rawPath) => {
    const s = stateRef.current;
    const path = String(rawPath).trim();
    if (!path) return { ok: false, error: 'Enter the path to the file to edit.' };

    const { suggestions, findings } = latestReview(s.messages, s.panel);
    if (suggestions.length === 0) {
      return {
        ok: false,
        error:
          'This review produced no code suggestions to apply. Run a review that ' +
          'returns before/after fixes first.',
      };
    }

    dispatch({ type: 'CODE_CHANGE_START', label: 'Preparing changes…' });

    const result = await postCodeChangesPreview({
      path,
      provider: s.provider,
      model: s.model,
      findings,
    ...
    if (!result.ok) {
      dispatch({ type: 'CODE_CHANGE_ERROR', error: result.error || 'The edit request failed.' });
      return { ok: false, error: result.error || 'The edit request failed.' };
    }

    dispatch({ type: 'CODE_CHANGE_PREVIEW', preview: result });
    return { ok: true };
  }, []);

  // Step 2 — write the confirmed proposal to disk (backend keeps a .bak).
  const writeCodeChanges = useCallback(async () => {
    const s = stateRef.current;
    const pending = s.pendingCodeChange;
    if (!pending) return { ok: false, error: 'There is nothing to write.' };

    dispatch({ type: 'CODE_CHANGE_START', label: 'Writing to disk…' });

    const result = await postCodeChangesApply({
      path: pending.path,
    ...
    if (!result.ok) {
      dispatch({
        ...
        error: result.error || 'The file could not be written.',
      });
      return { ok: false, error: result.error };
    }

    dispatch({ type: 'CODE_CHANGE_APPLIED', result });
    return { ok: true };
  }, []);

  const discardCodeChanges = useCallback(() => {
    const s = stateRef.current;
    const { suggestions } = latestReview(s.messages, s.panel);
    dispatch({ type: 'CODE_CHANGE_DISCARD', suggestions });
  }, []);
