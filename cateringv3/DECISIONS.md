...
- **Guard rail**: while building the admin app, never touch/read/write the customer app's pages or states (`customersec/{menu,cart,delivery_date,profile_drawer,containers,success}.py`, `state/{customerorderstate,customerpackingstate,stageoverlaystate}.py`). Shared primitives (`theme.py`, `components/{buttons,bottom_sheet,background}.py`) stay importable but unmodified. Why: explicit user instruction, added to CLAUDE.md 2026-07-28. (Note: the customer pages folder was renamed from `pages/` to `customersec/` at some point after this rule was first written — path updated here 2026-07-29 to match; a fresh `pages/` directory now holds admin-only pages with no collision.)
...
## Database (Postgres wiring)

- **`reflex db init` autogenerates *and applies* the first migration internally** (installed Reflex version 0.9.6.post2), so the plan's separate `makemigrations`/`migrate` steps were no-ops. The first attempt produced a generically-named `be9a6dc29da3_.py` ("empty message") — that file was later deleted and replaced (commit `345ec7e`) when a real bug was found (Postgres Enum columns storing uppercase member names instead of lowercase values; fixed via `values_callable` in `models.py`) and the migration was regenerated. The migration actually shipped on this branch is `alembic/versions/86936d25ff06_initial_schema.py`, properly named, verified column-by-column and FK-by-FK against `models.py`.
