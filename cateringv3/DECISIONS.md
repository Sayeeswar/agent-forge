...
- **First migration file is named `be9a6dc29da3_.py` with message "empty message"**, not a descriptive `initial_schema` name. Why: installed Reflex version (0.9.6.post2) makes `reflex db init` autogenerate *and apply* the first migration internally, so the planned separate `makemigrations --message "initial schema"` step became a no-op that couldn't set the message. Verified column-by-column against `models.py` (all 4 tables, all 3 FKs correct) before trusting it — not a sign of a bad migration, just a generic autogenerated name.
