...
5. **Apply fixes to disk** (local files only). The panel's "Make changes to the
   code" button asks for the file path, then the backend reads the file, asks
   the model to fold the review's before/after suggestions into it, and returns
   the **whole proposed file**. It is shown in the panel; nothing is written
   until you click **Write to disk**, which overwrites the file (keeping the
   original as `<file>.bak`) after a hash check that the file has not changed
   since the preview.
...
| `POST /api/code/changes/preview` | Rewrite a local file with the review's suggestions applied; writes nothing |
| `POST /api/code/changes/apply` | Write the confirmed rewrite to disk (`.bak` kept, staleness-checked) |
...
- **`backend/appsettings.json`** — tunables under `ApiKeys` (provider URLs,
  ...
  caps, temperature), `FileReview` (max file / image bytes), `CodeChange`
  (max file bytes, rewrite token cap, temperature).

## Not in scope yet

- Running the reviewed code — sandboxed execution is a separate module.
- Applying fixes to a **GitHub PR** — "Make changes to the code" is local-file
  only; PR sources are disabled for it.
